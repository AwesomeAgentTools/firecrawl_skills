...
If a task is "search the web for me right now", "scrape this URL during the session", "find papers", or "answer a library/API question from issues and docs", use the CLI skills (already installed alongside these build skills). The developer-index and research-index session skills live in [`firecrawl/cli`](https://github.com/firecrawl/cli).
