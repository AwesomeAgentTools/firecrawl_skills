...
- If the feature searches **published research papers** — biomedical, clinical, and life-science literature (PubMed, bioRxiv, medRxiv) or arXiv preprints — `/search` is the wrong surface. Use the research paper index (`firecrawl_research_*`, CLI `firecrawl research`). Session recipe: `firecrawl/cli` skill `firecrawl-research-index`. Passing `categories: ["research"]` to `/search` does **not** query that index; it filters an ordinary web search to research-affiliated websites (the list includes PubMed, bioRxiv, medRxiv, arXiv, and publisher sites) and returns page results from them — no abstract search, related-paper expansion, or full-text passages.
- If the feature answers developer questions from issues, pull requests, READMEs, or documentation pages, use the developer index (`GET` or `POST /v2/search/developer`, CLI `firecrawl developer`). Session recipe: `firecrawl/cli` skill `firecrawl-developer-index`. The same caveat applies to `categories: ["developer"]`.
